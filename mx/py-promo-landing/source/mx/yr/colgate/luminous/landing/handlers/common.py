...
from mx.yr.colgate.luminous.landing.forms import RegisterForm, \
     RedeemAccessoriesForm, RedeemTicketsForm, RedeemGameForm
from mx.yr.colgate.luminous.landing.models import Code, Points, Product, \
    ...
    def post(self):
        ...
        try:
            ...
            mail = self.do_send_mail(
                u"noreply@yr.com", 
                user.email, 
                u"Colgate Luminous White, Recupero de Contraseña",
    ...
    def get(self):
        ...
        redirect_uri = "%s://%s/auth/facebook?next=%s&terms=%s&policy=%s&news=%s" % (
                ...
                self.request.host if self.settings.get("debug")\
                                  else self.settings.get('colgate_domain'), 
    ...
    def do_reddem_code(self):
        ...
        try:
            ...
            message = (u"Su cuenta ha sido deshabilitada por haber alcanzado "
                       ...
                       u"contacto con Soporte (soporte@colgateluminouswhite.com.mx) para "
